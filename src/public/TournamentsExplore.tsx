...
export function TournamentsExplore() {
  ...
  useEffect(() => {
    ...
    // Calcular el precio máximo de los torneos
    const maxPrice = Math.max(...response.map(t => t.registrationCost), 0);
  ...
  const clearFilters = () => {
    ...
    const maxPrice = Math.max(...tournaments.map(t => t.registrationCost), 0);
    setPriceRange([0, maxPrice]);
  };
